...
def remove_noise(image, filter_type=None, kernel=None, size=5, sigma=75):
    ...
    def average(img): return cv2.blur(img, (size,size))
    def gaussian(img):  return cv2.GaussianBlur(img, (size, size), 0)
    def median(img): return cv2.medianBlur(img, size)
    def bilateral(img): return cv2.bilateralFilter(img, size, sigma, sigma)
...
def edge_detector(image, detect='canny', depth = cv2.CV_64F):
    ...
    def sobel(img, depth):
        ...
        return image_detect
    def laplacian(img, depth): return np.uint8(np.abs(cv2.Laplacian(img, depth)))
    def canny(img, depth): return cv2.Canny(img, 100, 200)
...
if __name__ == '__main__':
    ...
    kwargs = {'threshold': 150, 'size': 5, 'sigma': 75, 'depth': cv2.CV_64F}
    if params.kwargs:
